...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-16lvtr4_START -->${$$result.title = `<title>${escape(currentRoom.title)}</title>`, ""}<!-- HEAD_svelte-16lvtr4_END -->`, ""} <div class="${["container svelte-11yuiso", !!darkMode ? "dark" : ""].join(" ").trim()}"><div class="room-title svelte-11yuiso"><img${add_attribute("src", currentRoom.profilepicture, 0)} alt="pfp" class="room-picture svelte-11yuiso"> <span class="svelte-11yuiso">${escape(currentRoom.title)}</span></div> ${validate_component(MsgsContainer, "MsgsContainer").$$render($$result, { messages, currentRoomMembers }, {}, {})} ${validate_component(MsgInput, "MsgInput").$$render($$result, { id, socket }, {}, {})} </div>`;
});
export {
  Page as default
};
